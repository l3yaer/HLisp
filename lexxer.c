...
static hlisp_tok *make_token(hlisp_reader *reader, long start, long end, hlisp_type_t type, long parent)
{
...
hlisp_err_t parse_tok(hlisp_reader *reader, const char *str, size_t len)
{
    ...
    for(size_t i = 0; i < len;)
    {
        ...
        case '"':
            if(r == HLISP_ERR_OK)
                make_token(reader, start, end, HLISP_TOKEN_STRING, curr_tok_pos);
            break;
        case '-':
        case '0':
        case '1':
        ...
        case '9':
            if(r == HLISP_ERR_OK)
                make_token(reader, start, end, read_number_type(str, start, end), curr_tok_pos);
            break;
        case '(':
            curr_tok = make_token(reader, start, end, HLISP_TOKEN_LIST, curr_tok_pos);
        ...
        default:
            ...
            {
                make_token(reader, start, end, HLISP_TOKEN_NIL, curr_tok_pos);
                break;
            }

            if(strncmp(str + start, "true", 4) == 0 || strncmp(str + start, "false", 5) == 0)
            {
                make_token(reader, start, end, HLISP_TOKEN_BOOL, curr_tok_pos);
                break;
            }

            make_token(reader, start, end, HLISP_TOKEN_SYMBOL, curr_tok_pos);
            break;
        }
