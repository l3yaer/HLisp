...
static hlisp_tok_err_t lex(const char *str, size_t *start, size_t *end)
{
	const char *ws = " \t\n";
	*start += strspn(str, ws);
	if (str[(*start)] == '\0')
...
static hlisp_tok_t *make_token(struct hlisp_reader *reader, long start,
			       long end, hlisp_type_t type, long parent)
{
...
hlisp_tok_err_t parse_tok(struct hlisp_reader *reader, const char *str,
			  size_t len)
{
	...
	for (size_t i = 0; i < len;) {
		...
		case '(':
			...
			curr_tok_pos = reader_pos(reader) - 1;
			break;
		case ')':
			curr_tok->end = i;
			...
			if (curr_tok_pos == -1)
				...
				curr_tok = reader_tok(reader) + curr_tok_pos;
			break;
